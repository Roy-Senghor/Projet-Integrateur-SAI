...
def etat_actionneurs(db: Session = Depends(get_db), _user=Depends(get_current_user)):
    """Retourne le dernier état connu de chaque actionneur."""
    from sqlalchemy import func
    subq = (
        db.query(Action.actionneur, func.max(Action.timestamp).label("max_ts"))
        .group_by(Action.actionneur)
        .subquery()
    )
    return (
        db.query(Action)
        .join(subq, (Action.actionneur == subq.c.actionneur) & (Action.timestamp == subq.c.max_ts))
        .all()
    )
