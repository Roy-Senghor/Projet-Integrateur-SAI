import { NavLink } from 'react-router-dom'
import { useAuth } from '../context/AuthContext'

const ROLE_LABELS = {
  admin:        '👑 Administrateur',
  modification: '✏️ Modificateur',
  consultation: '👁️ Consultation',
}

const Sidebar = ({ isOpen, onClose, alertCount = 0 }) => {
  const { logout, role, isAdmin, isModification } = useAuth()

  return (
    <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
      <button className="sidebar-close-btn" onClick={onClose}>×</button>

      {/* Logo */}
      <div className="sidebar-header">
        <div className="logo-mark">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
            <path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/>
            <path d="M2 21c0-3 1.85-5.36 5.08-6C9.5 14.52 12 13 13 12"/>
          </svg>
        </div>
        <div className="logo-text">
          <h2>AgroSmart</h2>
          <small>Agriculture Intelligente</small>
        </div>
      </div>

      <nav className="sidebar-nav">
        <div className="nav-section-label">Surveillance</div>

        {/* Dashboard — tous */}
        <NavLink to="/dashboard" end className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/>
            <rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/>
          </svg>
          Dashboard
        </NavLink>

        {/* Actionneurs — tous (consultation = lecture seule) */}
        <NavLink to="/actionneurs" className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <circle cx="12" cy="12" r="3"/>
            <path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12"/>
          </svg>
          Actionneurs
          {!isModification && <span style={{ marginLeft:'auto', fontSize:'0.6rem', opacity:0.6 }}>👁️</span>}
        </NavLink>

        {/* Seuils — tous (consultation = lecture seule) */}
        <NavLink to="/seuils" className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M2 20h20"/><path d="M6 20V10"/><path d="M10 20V4"/><path d="M14 20V14"/><path d="M18 20V8"/>
          </svg>
          Seuils d'alerte
          {!isModification && <span style={{ marginLeft:'auto', fontSize:'0.6rem', opacity:0.6 }}>👁️</span>}
        </NavLink>

        <div className="nav-section-label">Gestion</div>

        {/* Historique — tous */}
        <NavLink to="/historique" className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
          </svg>
          Historique
          {alertCount > 0 && <span className="nav-badge">{alertCount}</span>}
        </NavLink>

        <NavLink to="/parametres" className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/>
            <circle cx="12" cy="12" r="3"/>
          </svg>
          Paramètres
        </NavLink>

        {/* Utilisateurs — admin seulement */}
        {isAdmin && (
          <>
            <div className="nav-section-label">Administration</div>
            <NavLink to="/utilisateurs" className={({ isActive }) => `nav-link${isActive ? ' active' : ''}`}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
                <circle cx="9" cy="7" r="4"/>
                <path d="M23 21v-2a4 4 0 0 0-3-3.87"/>
                <path d="M16 3.13a4 4 0 0 1 0 7.75"/>
              </svg>
              Utilisateurs
            </NavLink>
          </>
        )}

        {/* Statut système */}
        <div className="sidebar-status" style={{ marginTop: 16 }}>
          <div className="status-header">
            <span className="status-title">Système</span>
            <span className="status-indicator">
              <span className="status-dot"/>En ligne
            </span>
          </div>
          <div className="status-row">
            <span>Capteurs actifs</span><span className="status-val">6/6</span>
          </div>
          <div className="status-row">
            <span>Votre rôle</span>
            <span className="status-val" style={{ fontSize: '0.68rem' }}>
              {role || '—'}
            </span>
          </div>
        </div>
      </nav>

      {/* User footer */}
      <div className="sidebar-user">
        <div className="user-avatar">
          {role === 'admin' ? '👑' : role === 'modification' ? '✏️' : '👁️'}
        </div>
        <div className="user-info">
          <div className="user-name">{ROLE_LABELS[role] || 'Utilisateur'}</div>
          <div className="user-role">Connecté</div>
        </div>
        <button className="btn-logout" onClick={logout} title="Déconnexion">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/>
            <polyline points="16 17 21 12 16 7"/>
            <line x1="21" y1="12" x2="9" y2="12"/>
          </svg>
        </button>
      </div>
    </aside>
  )
}

export default Sidebar
